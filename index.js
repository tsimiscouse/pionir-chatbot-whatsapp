...
client.on('message', async (message) => {
    try {
        ...
        if (userSession.conversationState === 'initial') {
            const greetingRule = faqData.find(rule => rule.regex.test(messageText) && rule.answer.includes("Halo GAMADA!"));
            if (greetingRule) {
            ...
            if (nameRule) {
                ...
                if (match && match[2]) {
                 ...
                 await message.reply("Maaf, nama yang kamu berikan tidak terdeteksi. Tolong coba lagi dengan format seperti 'nama saya [nama kamu]'.");
                 responseSent = true;
            }
        }
